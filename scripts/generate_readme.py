#!/usr/bin/env python3
"""
Generate AUTO_DOCUMENTATION.md from package.json and docs/documentation.json.
...
def parse_args(argv: list[str] | None = None) -> Paths:
    parser = argparse.ArgumentParser(
        description="Generate AUTO_DOCUMENTATION.md from extension metadata."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output markdown path (default: <root>/AUTO_DOCUMENTATION.md)",
    )
    parser.add_argument(
        "--version",
        default=None,
    ...
    return Paths(
        ...
        output=(args.output or root / "AUTO_DOCUMENTATION.md").resolve(),
...
def render_banner(paths: Paths) -> str:
  ...
  Sources: {paths.package_json.name}, {paths.documentation_json.relative_to(paths.root)}
-->
"""
