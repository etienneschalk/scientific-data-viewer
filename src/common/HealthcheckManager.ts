...
import { getAllowMultipleTabsForSameFile, getDevMode, getMatplotlibStyle, getMaxSize, getOverridePythonInterpreter, getUseExtensionOwnEnvironment, getWebviewExportTheme } from './config';
...
export class HealthcheckManager {
    ...
    public async runHealthcheck(
        ...
        const supportedFormatsResult = await this.checkSupportedFormats(pythonManager);
     ...
     */
    public generateMarkdownReport(report: HealthcheckReport, pythonManager: PythonManager): string {
        const statusIcon = {
            healthy: '✅',
            warning: '⚠️',
            error: '❌',
        ...
        markdown += `- **Overall Status:** ${statusIcon[report.overallStatus]} ${statusColor[report.overallStatus]} ${report.overallStatus.toUpperCase()}\n\n`;
        ...
        markdown += `- **Extension Path:** ${vscode.extensions.getExtension('eschalk0.scientific-data-viewer')?.extensionPath || 'Unknown'}\n\n`;
        
        // Environment Information
        markdown += `## 🐍 Environment Information\n\n`;
        const envInfo = pythonManager.getCurrentEnvironmentInfo();
        if (envInfo) {
            markdown += `- **Status:** ${envInfo.initialized ? (envInfo.ready ? '✅ Ready' : '⚠️ Not Ready') : '❌ Not Initialized'}\n`;
            markdown += `- **Source:** ${envInfo.source || 'Unknown'}\n`;
            markdown += `- **Python Path:** ${envInfo.path || 'Not set'}\n`;
            markdown += `- **Initialized:** ${envInfo.initialized ? 'Yes' : 'No'}\n`;
        ...
        const errorIssues = report.issues.filter(issue => issue.status === 'error');
        const warningIssues = report.issues.filter(issue => issue.status === 'warning');
        const healthyIssues = report.issues.filter(issue => issue.status === 'healthy');
     ...
     */
    public async openHealthcheckReport(report: HealthcheckReport, pythonManager: PythonManager): Promise<void> {
    ...
    private checkPanes(): HealthcheckResult {
        ...
        if (paneCount === 0) {
            return {
                ...
                details: 'Consider opening a scientific data file to test the extension functionality.',
                timestamp: new Date(),
            };
        }
        ...
        if (paneCount > 10) {
            return {
                ...
                details: 'Having many open panes may impact performance. Consider closing unused panes.',
                timestamp: new Date(),
            };
        }
    ...
    private checkPanesWithErrors(): HealthcheckResult {
        try {
            ...
            if (panesWithErrors > 0) {
                const errorDetails = errorPanels.map(panel => 
                    `- ⚠️ Panel ${panel.getId()}: ${panel.getFileUri().fsPath}`
                ).join('\n');

                return {
                    status: 'warning',
                    message: `${panesWithErrors} of ${totalPanes} panes have errors`,
                    details: `Some data viewer panes encountered errors while loading data.\n\n` +
    ...
    private async checkPythonEnvironment(pythonManager: PythonManager): Promise<HealthcheckResult> {
        try {
            const envInfo = pythonManager.getCurrentEnvironmentInfo();
            ...
            if (!envInfo) {
                return {
                    ...
                    details: 'The Python environment has not been properly initialized. Try refreshing the Python environment.',
                    timestamp: new Date(),
                };
            }
            ...
            if (!envInfo.initialized) {
                return {
                    ...
                    details: 'The Python environment is still being initialized. This is normal during startup.',
                    timestamp: new Date(),
                };
            }
            ...
            if (!envInfo.ready) {
                return {
                    ...
                    details: `Python environment is initialized but missing core packages. Source: ${envInfo.source || 'unknown'}. Path: ${envInfo.path || 'unknown'}`,
                    timestamp: new Date(),
                };
            }
            ...
            return {
                status: 'healthy',
                message: `Python environment ready (${envInfo.source || 'unknown'})`,
    ...
    private async checkDependencies(pythonManager: PythonManager): Promise<HealthcheckResult> {
        try {
            const corePackages = ['xarray'];
            const plotPackages = ['matplotlib'];
            const extendedPackages = ['netCDF4', 'h5netcdf', 'zarr', 'h5py', 'scipy', 'cfgrib', 'rioxarray'];

            const allPackages = [...corePackages, ...plotPackages, ...extendedPackages];
            
            if (!pythonManager.pythonPath) {
                return {
                    status: 'error',
                    message: 'Python interpreter not available',
                    details: 'Cannot check package availability without a Python interpreter.',
                    timestamp: new Date(),
                };
            }

            const packageAvailability = await pythonManager.checkPackagesAvailability(
            ...
            const coreMissing = missingPackages.filter(pkg => corePackages.includes(pkg));
            const plotMissing = missingPackages.filter(pkg => plotPackages.includes(pkg));
            const extendedMissing = missingPackages.filter(pkg => extendedPackages.includes(pkg));

            // Create detailed package lists
            const coreAvailable = corePackages.filter(pkg => packageAvailability[pkg] === true);
            const plotAvailable = plotPackages.filter(pkg => packageAvailability[pkg] === true);
            const extendedAvailable = extendedPackages.filter(pkg => packageAvailability[pkg] === true);
            ...
            if (coreMissing.length > 0) {
                ...
                details = `Core packages are required for basic functionality.\n\n` +
                    `- ✅ Available core packages: ${coreAvailable.length > 0 ? coreAvailable.join(', ') : 'None'}\n` +
                    `- ❌ Missing core packages: ${coreMissing.join(', ')}\n` +
                    `- ✅ Available plotting packages: ${plotAvailable.length > 0 ? plotAvailable.join(', ') : 'None'}\n` +
                    `- ⚠️ Missing plotting packages: ${plotMissing.length > 0 ? plotMissing.join(', ') : 'None'}\n` +
                    `- ✅ Available extended packages: ${extendedAvailable.length > 0 ? extendedAvailable.join(', ') : 'None'}\n` +
                    `- ⚠️ Missing extended packages: ${extendedMissing.length > 0 ? extendedMissing.join(', ') : 'None'}`;
            } else if (plotMissing.length > 0 || extendedMissing.length > 0) {
                status = 'warning';
                message = `Missing optional packages: ${[...plotMissing, ...extendedMissing].join(', ')}`;
                details = `All core packages are available, but some optional packages are missing.\n\n` +
                ...
                details = `- ✅ Available core packages: ${coreAvailable.join(', ')}\n` +
    ...
    private async checkConfiguration(): Promise<HealthcheckResult> {
        try {
            ...
            if (devMode) {
                warnings.push('Development mode is enabled (this is normal for development)');
            }
            ...
            if (useExtensionOwnEnv && !overridePython) {
                ...
                if (!uvAvailable) {
                    issues.push('Using extension own environment but uv is not available');
                } else {
                    warnings.push('Using extension own environment (uv is available)');
                }
            }
            ...
            if (issues.length > 0) {
                return {
                    ...
                    details: `Errors: ${issues.join('; ')}${warnings.length > 0 ? ` Warnings: ${warnings.join('; ')}` : ''}`,
                    timestamp: new Date(),
                };
            } else {
                return {
    ...
    private async checkUvAvailability(): Promise<boolean> {
        try {
            ...
            try {
                ...
                Logger.debug(`uv is not available: ${error instanceof Error ? error.message : String(error)}`);
                return false;
            }
        } catch (error) {
            Logger.debug(`Failed to check uv availability: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    ...
    private checkErrorBoundary(): HealthcheckResult {
        try {
            ...
            return {
                ...
                details: 'Error boundary is properly initialized and monitoring for errors.',
                timestamp: new Date(),
            };
        } catch (error) {
            return {
    ...
    private generateRecommendations(issues: HealthcheckResult[], recommendations: string[]): void {
        const errorIssues = issues.filter(issue => issue.status === 'error');
        const warningIssues = issues.filter(issue => issue.status === 'warning');

        if (errorIssues.length > 0) {
            recommendations.push('🚨 **URGENT**: Address all error-level issues first as they may prevent the extension from working properly');
        }

        if (errorIssues.some(issue => issue.message.includes('Python environment'))) {
            recommendations.push('🔧 **Error Fix**: Try refreshing the Python environment using the "Refresh Python Environment" command');
        }

        if (errorIssues.some(issue => issue.message.includes('core packages'))) {
            recommendations.push('🔧 **Error Fix**: Install missing core packages using the "Install Python Packages" command or refresh the Python environment');
        }

        if (warningIssues.some(issue => issue.message.includes('packages'))) {
            recommendations.push('⚠️ **Warning**: Consider installing missing optional packages for enhanced functionality');
        }

        if (warningIssues.some(issue => issue.message.includes('panes'))) {
            recommendations.push('⚠️ **Warning**: Consider closing unused data viewer panes to improve performance');
        }

        if (warningIssues.some(issue => issue.message.includes('memory'))) {
            recommendations.push('⚠️ **Warning**: Consider restarting VS Code if memory usage continues to be high');
        }

        if (warningIssues.some(issue => issue.message.includes('configuration'))) {
            recommendations.push('⚠️ **Warning**: Review your extension configuration settings');
        }

        if (recommendations.length === 0) {
            recommendations.push('✅ No specific recommendations at this time - all systems are healthy!');
        }
    }

    private async checkSupportedFormats(pythonManager: PythonManager): Promise<HealthcheckResult> {
        try {
            ...
            if (!pythonManager.pythonPath) {
                return {
                    status: 'error',
                    message: 'Cannot check supported formats without Python interpreter',
                    details: 'Python interpreter is required to check format support.',
                    timestamp: new Date(),
                };
            }

            // Collect all unique packages needed for engines
            const allEnginePackages = [...new Set(Object.values(formatEngines).flat())];
            ...
            for (const [engine, packages] of Object.entries(formatEngines)) {
                const allPackagesAvailable = packages.every(pkg => packageAvailability[pkg] === true);
            ...
            if (availableEngineCount === 0) {
                return {
                    ...
                    details: `No data format engines are available.\n\n` +
            ...
            if (availableEngineCount < totalEngines) {
                return {
                    ...
                    details: `Some format engines are missing, limiting data format support.\n\n` +
            ...
            return {
                ...
                details: `All format engines are available for comprehensive data format support.\n\n` +
                    `- ✅ Available engines: ${availableEngines.join(', ')}`,
                timestamp: new Date(),
            };
        } catch (error) {
    ...
    private determineOverallStatus(issues: HealthcheckResult[]): 'healthy' | 'warning' | 'error' {
