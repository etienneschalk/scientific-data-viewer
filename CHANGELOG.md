...
- **Issue #131**: Performance improvements for faster file-open readiness (time from opening a scientific file to data displayed).
  ...
  - **Loading status while fetching**: Shows “Loading data…” or “Refreshing data…” (and filename when known) while Python fetches metadata; driven by extension `uiStateChanged` when `isLoading`.
  - **Incremental group rendering**: Multi-group files with more than 4 groups render in `requestAnimationFrame` batches to keep the webview responsive.
  - **Load-testing sample data**: `sample_data_many_groups_light.nc` (28 lightweight groups) and `sample_data_many_groups_light_x5.nc` (140 groups) for manual multi-group UI testing; generated by `create_sample_data.py` / `setup.sh`.
...
- **Tests** for the above:
  ...
  - Python: `python/test_performance_features.py` (`dispatch_argv` skip-reprs/repr modes, worker ping/execute), `python/test_datetime_min_max.py` (`datetime_min_max_iso`), `python/test_many_groups_light_sample.py` (many-groups light sample NetCDF files)
...
- **Issue #139 — Refresh button**: Full reload now resets the webview (groups, repr sections, plots, global plot controls), bypasses and invalidates the metadata cache, cancels in-flight plots, and avoids duplicate lazy-repr listeners on repeated refresh. The refresh and export-webview buttons are disabled while their operation is in progress.
