...
- **Issue #117**: Select and slice dimensions in the UI before plotting (xarray-style isel and faceting)
  ...
  - **Global vs. group inheritance:** When plotting a variable, the extension uses that variable’s group state when the group has any value set, else falls back to global. Per field, group when set else global—so the two can be mixed. The only atomic case is **dimension slices**: one set of inputs; we use either the whole group’s dimension-slice set or the whole global (no merge). One change in a group's dimension slice inputs means that group's full set is used and global is ignored for that group; other params still fall back to global per field.
